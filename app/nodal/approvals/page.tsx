...
const INIT: Rx[] = [
  {
    id: "RX-001",
    patientName: "Ramaiah Venkata",
    age: 58,
    village: "Patamata",
    diagnosis: "Presbyopia",
    power: "OD: +1.25/-0.50×90, OS: +1.00/-0.25×80",
    date: "02 Jun 2025",
  ...
  },
  {
    ...
    village: "Ongole",
    diagnosis: "Cataract",
    power: "Surgical referral recommended",
    date: "01 Jun 2025",
  ...
  },
  {
    ...
    village: "Kurnool",
    diagnosis: "Myopia",
    power: "OD: -2.50/-0.75×175, OS: -2.00/-0.50×170",
    date: "02 Jun 2025",
    doctor: "Dr. Srinivasa Rao",
    status: "Approved",
  },
  {
    ...
    power: "OD: -1.00/-1.25×90, OS: -1.25/-1.00×85",
    date: "31 May 2025",
...
export default function ApprovalScreen() {
  ...
  const filtered = rxList.filter(
    (p) => filter === "All" || p.status === filter,
  );

  const doAction = () => {
    if (!actionRx) return;
    if (actionType === "reject" && !rejectRemarks.trim()) return;
    setRxList((prev) =>
      prev.map((r) =>
        r.id === actionRx.id
          ? {
    ...
    <AppShell
      ...
      topbar={
        <Topbar
          ...
          subtitle="Review and approve prescriptions"
        />
      }
    >
      <div className="mb-4 flex gap-2">
        {["Pending", "Approved", "All"].map((s) => (
          <Button
            ...
            className={cn(
              filter !== s && "border-grey-300 bg-white text-grey-600",
            )}
            onClick={() => setFilter(s)}
          >
            ...
            <CardBody>
              <div className="mb-2 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="flex size-10 items-center justify-center rounded-full bg-primary/10 text-base font-extrabold text-primary">
                    {rx.patientName[0]}
                  </div>
                  <div>
              ...
              <div className="mb-3 rounded-[10px] bg-grey-50 p-3">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-[13px] font-bold">{rx.doctor}</div>
                  </div>
                  <div className="col-span-2">
                    <div className="text-[10px] font-bold text-grey-400">
                      POWER
                    </div>
                    <div className="text-xs text-grey-800">{rx.power}</div>
                  </div>
                </div>
              </div>
              {rx.status === "Pending" ? (
        ...
        actions={
          <>
            <Button
              ...
              size="sm"
              onClick={() => {
                viewRx && handlePrint(viewRx);
              }}
            >
              🖨️ Print
            </Button>
            <Button variant="primary" size="sm" onClick={() => setViewRx(null)}>
              Close
            </Button>
          </>
        }
      >
        {viewRx && (
          <>
            <DetailRow
              label="Patient"
              value={`${viewRx.patientName}, ${viewRx.age} yrs`}
            />
          ...
          !done ? (
            <>
              ...
              <Button
                ...
                onClick={doAction}
                disabled={actionType === "reject" && !rejectRemarks.trim()}
              >
                {actionType === "approve"
                  ? "✅ Confirm Approve"
                  : "❌ Confirm Reject"}
      ...
      >
        {done ? (
          <SuccessBanner
            message={`Prescription ${actionType === "approve" ? "approved" : "rejected"} successfully!`}
          />
        ) : (
          actionRx && (
            <div className="py-3">
              ...
              <div className="text-center text-sm font-bold">
                ...
                for {actionRx.patientName}?
              </div>
              {actionType === "reject" && (
                <textarea
                  className="mt-4 w-full rounded-lg border border-grey-300 p-3 text-sm"
                  rows={3}
