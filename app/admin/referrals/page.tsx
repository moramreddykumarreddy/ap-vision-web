...
export default function ReferralManagement() {
  ...
  const doSubmitNew = () => {
    ...
    const n: Ref = {
      id: `REF-00${refs.length + 1}`,
      patient: newRef.patient,
      age: Number(newRef.age) || 0,
      condition: newRef.condition,
      hospital: newRef.hospital,
    ...
    <AppShell
      ...
      topbar={
        <Topbar
          ...
          subtitle="Hospital and specialist referrals"
        />
      }
    >
            ...
            <DataTable>
              ...
              <tbody>
                {refs.map((r, i) => (
                  <tr
                    key={r.id}
                    className={`animate-fade-up ${fadeDelay(i + 1)}`}
                  >
                    <td className="font-bold">{r.id}</td>
                    <td>
                      <div className="font-bold">{r.patient}</div>
                      <div className="text-[11px] text-grey-400">{r.age}y</div>
                    </td>
                    <td>{r.condition}</td>
                    ...
                    <td>
                      <span
                        className={`inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${priorityBadgeClass(r.priority)}`}
                      >
                        {r.priority}
                      </span>
                    </td>
      ...
      <Modal
        ...
        subtitle={viewRef?.id}
        actions={
          <Button variant="primary" size="sm" onClick={() => setViewRef(null)}>
            Close
          </Button>
        }
      >
        {viewRef && (
          <>
            ...
            <DetailRow
              label="Patient"
              value={`${viewRef.patient}, ${viewRef.age} yrs`}
            />
      ...
      <Modal
        open={!!approveRef}
        onClose={() => setApproveRef(null)}
        title="Approve Referral"
        subtitle={`${approveRef?.patient} — ${approveRef?.condition}`}
        actions={
          !approvedId ? (
            <>
              <Button
              ...
              <Button
                variant="primary"
                size="sm"
                onClick={() => approveRef && doApprove(approveRef.id)}
              >
                ✅ Confirm Approve
              </Button>
            </>
      ...
      >
        {approvedId ? (
          <SuccessBanner
            message={`Referral ${approvedId} approved successfully!`}
          />
        ) : (
          approveRef && (
            <>
              <DetailRow
                label="Patient"
                value={`${approveRef.patient}, ${approveRef.age} yrs`}
              />
              ...
              <div className="mt-3.5 rounded-[10px] bg-primary/5 p-3.5 text-xs text-grey-800">
                Approving this referral will notify the hospital and send
                appointment details to the patient via SMS.
              </div>
            </>
          )
        )}
        ...
        actions={
          !submitted ? (
            <>
              ...
              <Button variant="primary" size="sm" onClick={doSubmitNew}>
                Submit Referral
              </Button>
            </>
          ) : undefined
      ...
      >
        {submitted ? (
          <SuccessBanner message="Referral submitted successfully!" />
        ) : (
          <div>
            {[
              {
              ...
              <FormGroup key={f.key} label={f.label}>
                <Input
                  ...
                  value={(newRef as Record<string, string>)[f.key]}
                  onChange={(e) =>
                    setNewRef((p) => ({ ...p, [f.key]: e.target.value }))
                  }
                />
              </FormGroup>
            ))}
            <FormGroup label="Priority">
              <Select
                ...
                onChange={(e) =>
                  setNewRef((p) => ({ ...p, priority: e.target.value }))
                }
              >
