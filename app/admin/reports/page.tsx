...
const reports = [
  {
    id: "RPT-001",
    title: "Monthly Screening Report – May 2025",
    type: "Monthly",
  ...
  {
    id: "RPT-002",
    title: "District Coverage Report – Krishna",
    type: "District",
    district: "Krishna",
    date: "28 May 2025",
    size: "880 KB",
  ...
  {
    id: "RPT-003",
    title: "Spectacle Delivery Report – Q1 2025",
    type: "Delivery",
    district: "All Districts",
    date: "01 Apr 2025",
    size: "1.2 MB",
  ...
  {
    id: "RPT-004",
    title: "Annual Programme Report 2024–25",
    type: "Annual",
    district: "State-wide",
    date: "31 Mar 2025",
    size: "8.6 MB",
  ...
  {
    id: "RPT-005",
    title: "Teleconsultation Utilization Report",
    type: "Specialty",
    district: "All Districts",
    date: "15 May 2025",
    size: "640 KB",
...
const reportTypes = [
  {
    icon: "📊",
    label: "State Summary Report",
    desc: "Overall KPIs and progress",
  },
  { icon: "🗺️", label: "District Report", desc: "District-wise breakdown" },
  {
    icon: "⛺",
    label: "Camp-wise Report",
    desc: "Individual camp performance",
  },
  {
    icon: "👓",
    label: "Spectacle Delivery",
    desc: "Spectacle order and delivery stats",
  },
  { icon: "📹", label: "Tele-consult Report", desc: "Consultation statistics" },
  {
    icon: "💊",
    label: "Prescription Report",
    desc: "All prescriptions summary",
  },
];
...
export default function ReportsScreen() {
  ...
  const [shareReport, setShareReport] = useState<(typeof reports)[0] | null>(
    null,
  );
  ...
  const handleDownload = (r: (typeof reports)[0]) => {
    downloadFile(
      `${r.id}_${r.title.replace(/\s+/g, "_")}.csv`,
      CSV_DATA,
      "text/csv",
    );
  };
    ...
    >
      ...
      <div className="mt-2.5 mb-3 grid grid-cols-1 gap-3.5 sm:grid-cols-2 lg:grid-cols-3">
        {reportTypes.map((r, i) => (
          <Card
            key={r.label}
            className={`animate-fade-up cursor-pointer transition-all duration-200 hover:-translate-y-0.5 ${fadeDelay(i + 1)}`}
            onClick={() => handleGenerate(r.label)}
          >
            <CardBody className="flex items-center gap-3.5">
              <div className="flex size-10 shrink-0 items-center justify-center rounded-[10px] bg-primary/10 text-xl">
                {r.icon}
              </div>
              <div>
                <div className="text-[13px] font-bold">{r.label}</div>
                <div className="text-[11px] text-grey-400">{r.desc}</div>
              </div>
            </CardBody>
          </Card>
        ))}
      ...
      <Card className="mb-3">
        <CardBody>
          ...
          <div className="grid grid-cols-1 items-end gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <FormGroup label="From Date" className="mb-0">
              <Input
                type="date"
                value={fromDate}
            ...
            <FormGroup label="To Date" className="mb-0">
              <Input
                type="date"
                value={toDate}
        ...
        <CardBody className="p-0">
          {reports.map((r, i) => (
            <ListItem
              key={r.id}
              avatar="📄"
              avatarBg="colorAlpha(colors.primary, '10')"
          ...
          generated ? (
            <>
              <Button
                ...
                size="sm"
                onClick={() =>
                  downloadFile("generated_report.csv", CSV_DATA, "text/csv")
                }
              >
                📥 Download CSV
              </Button>
      ...
      >
        {generating ? (
          <div className="py-6 text-center">
            ...
            <div className="mt-1.5 text-xs text-grey-400">
              Processing data from all districts...
            </div>
          </div>
        ) : generated ? (
          <div>
            ...
            <div className="mt-4 rounded-[10px] bg-grey-50 p-3.5">
              <div className="mb-2.5 text-xs font-bold text-primary">
                Report Preview
              </div>
              <pre className="whitespace-pre-wrap text-[11px] leading-relaxed text-grey-800">
                {CSV_DATA.slice(0, 400)}...
              </pre>
            </div>
          </div>
        ) : null}
      ...
      >
        {shareReport && (
          <div>
            <div className="mb-4 flex items-center gap-2.5 rounded-[10px] bg-primary-container px-4 py-3">
              <code className="flex-1 break-all text-[11px] text-primary">
                https://apvision.gov.in/reports/{shareReport.id}
              </code>
              <Button
                size="sm"
                variant="primary"
            ...
            {shareCopied && (
              <SuccessBanner message="Link copied to clipboard!" />
            )}
            <div className="mt-3.5 flex gap-2.5">
              {["📧 Email", "📱 WhatsApp", "📨 SMS"].map((m) => (
                <Button key={m} variant="outline" size="sm" className="flex-1">
                  {m}
                </Button>
              ))}
            </div>
