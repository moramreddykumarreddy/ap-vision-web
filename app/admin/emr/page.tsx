"use client";
import { colors, emrTypeColors, emrTypeIcons } from "@/app/lib/theme";
...
export default function EmrTimeline() {
  ...
  const handleAdd = () => {
    ...
    const n: Event = {
      ...
      type: addType,
      title: addTitle,
      doctor: addDoctor || "Dr. Attending",
      summary: addSummary || "Entry added via admin portal.",
      color: emrTypeColors[addType] ?? colors.primary,
      icon: emrTypeIcons[addType] ?? "📋",
    ...
    >
      <Card className="mb-2.5">
        <CardBody>
          <div className="flex items-center gap-3.5">
            <div className="flex size-[50px] shrink-0 items-center justify-center rounded-full bg-primary text-xl font-black text-white">
              R
            </div>
            <div className="min-w-0 flex-1">
              <div className="text-base font-black">Ramaiah Venkata</div>
              <div className="text-xs text-grey-400">
                58 years • Male • APV-001234 • Patamata, Krishna
              </div>
              <div className="mt-1.5 flex flex-wrap gap-2">
                ...
                <span className="inline-flex rounded-full bg-warning/10 px-2.5 py-0.5 text-[11px] font-semibold text-warning">
                  Diabetic
                </span>
              </div>
            </div>
            <div className="flex gap-2">
              <Button
                ...
                size="sm"
                onClick={() =>
                  downloadFile("EMR_Ramaiah_Venkata_APV001234.txt", EMR_TEXT)
                }
              >
                📄 Download EMR
              </Button>
      ...
      <div className="pl-7">
        {allEvents.map((e, i) => (
          <div
            key={i}
            className={`relative animate-fade-up pb-4 pl-5 ${fadeDelay(i + 1)}`}
          >
            <div
              className="absolute -left-[7px] top-1 size-4 rounded-full"
              style={{
                background: e.color,
                boxShadow: `0 0 0 3px ${e.color}30`,
              }}
            />
            {i < allEvents.length - 1 && (
              <div className="absolute -left-px top-5 h-[calc(100%-12px)] w-0.5 bg-grey-200" />
            )}
            <Card>
              <CardBody>
                <div className="mb-2">
                ...
                <div className="rounded-lg bg-grey-50 px-3.5 py-2.5 text-xs text-grey-800">
                  {e.summary}
                </div>
                <Button
                  variant="ghost"
      ...
      <Modal
        ...
        subtitle={`${viewEvent?.type} • ${viewEvent?.date}`}
        actions={
          <Button
            variant="primary"
            size="sm"
      ...
      >
        {viewEvent && (
          <div>
            ...
            <div className="mt-3.5 whitespace-pre-wrap rounded-[10px] bg-grey-50 p-3.5 text-xs leading-relaxed text-grey-800">
              {viewEvent.details}
            </div>
          </div>
        )}
      ...
      >
        {added ? (
          <SuccessBanner message="EMR entry added successfully!" />
        ) : (
          <div>
