    ...
    >
      ...
      <div className="mt-3.5">
        <SectionHeader
          ...
          onAction={() => router.push("/admin/analytics/district")}
        />
        <Card>
          <CardBody className="p-0">
            {districts.map((d, i) => (
              <div
                key={d.name}
                className={`flex animate-fade-up items-center gap-3 border-b border-grey-100 px-5 py-3.5 last:border-b-0 ${fadeDelay(i + 1)}`}
              >
                <RankBadge rank={i + 1} />
                <div className="min-w-0 flex-1">
                  <div className="text-[13px] font-bold">{d.name}</div>
                  <div className="text-[11px] text-grey-400">
                    {d.screened.toLocaleString()} screened
                  </div>
                </div>
